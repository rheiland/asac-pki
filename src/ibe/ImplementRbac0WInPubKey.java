...
public class ImplementRbac0WInPubKey extends Implementation {

    public static final String SU = "SU";
    static final String TRUE = PubKey.TRUE;
    static final String FALSE = PubKey.FALSE;
    ...
    public void init(WorkloadState wState) {
        ...
        wScheme.sys = sys;
        scheme = new PubKey(stateMap(wState));
    }
    ...
    public State stateMap(WorkloadState wState) {
        ...
        PubKeyState pState = new PubKeyState();

        pState.addRoleKey(SU, SU);

        for(String u : rState.U) {
            Log.v("Mapping user " + u + " to initial PubKey state");
            pState.addRoleKey(u, u);
        }

        for(String p : rState.P) {
            Log.v("Mapping file " + p + " to initial PubKey state");
            pState.addFile(p);
            pState.addKey(p, SU, true, true, 1);
        }

        for(String r : rState.R) {
            Log.v("Mapping role " + r + " to initial PubKey state");
            pState.addRoleKey(SU, r);
        }

        for(String u : rState.UA.keySet()) {
            for(String r : rState.UA.get(u)) {
                Log.v("Giving key " + r + " to user " + u + " in initial PubKey state");
        ...
        for(String p : rState.PA.keySet()) {
            for(String r : rState.PA.get(p)) {
                ...
                Log.v("Assigning " + p + " to role " + r + " in initial PubKey state");
    ...
    public void action(Action a) {
        PubKey pubkey = (PubKey) scheme;
        PubKeyState pState = (PubKeyState) pubkey.state;
        super.action(a);
        ...
        switch(a.name) {
            ...
            case "revokePermission": {
                ...
                PubKeyRing kr = pState.keys.get(f).get(r);
