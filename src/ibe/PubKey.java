...
public class PubKey extends Scheme {

    public PubKey(State initState) {
        super(initState);

        if(!(initState instanceof PubKeyState)) {
    ...
    public static final Property lastStateProperty = new Property(PubKey.class, Scheme.Q_LASTSTATE, new String[0]);
    ...
    private void command(Action acAction) {
        PubKeyState iState = (PubKeyState) state;
    ...
    private void query(Action acAction) {
        ...
        switch(acAction.name) {
            ...
            case "write": {
                ...
                int v = iState.files.get(f);
                Set<String> uid = new HashSet<String>(iState.roleKeys.get(u));
                Map<String, PubKeyRing> fkeys = iState.keys.get(f);
