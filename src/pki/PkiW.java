...
public class PkiW extends WorkloadScheme {

    public PkiW(WorkloadState initState) {
        super(initState);
    }
    ...
    private void command(Action acAction) {
        PkiWState rState = (PkiWState) state;
        Log.d("PkiW:command: action=" + acAction.name);
//        java.lang.System.out.println("(rwh)PkiW.command(): " + acAction.name);
        switch(acAction.name) {
//        case "checkDateTimeValid":
        case "checkPathValidation":
//            sys.addActor(name, am);
//            sys.addActor(name, am);  // rwh: 'sys' is public in base/WorkloadScheme
            // Constant time to: get the current date and check it against the cert's start/end valid time.
            break;
...
//                break;
    ...
    private void query(Action acAction) {
        PkiWState rState = (PkiWState) state;
        String u, r, p;
        Log.d("PkiW:query: action=" + acAction.name);
        switch(acAction.name) {
            case "isAuthorized": {
                Log.d("PkiW:query: action=" + acAction.name);
                return;
...
//        "UR", "PA", "auth"
    ...
    public Action addParameters(Action a, Workflow w) {
        PkiWState rState = (PkiWState) state;
        Log.d("PkiW:addParameters:  action=" + a.name);
        if(a.name.equals("addU")) {
            ...
            Log.w("PkiW: No recipe to add parameters to action " + a);
            return a;
        }
    }
