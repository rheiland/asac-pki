...
public class ActorManager extends ActorMachine {
	...
	public Action nextAction(Params params) {
		...
		//dT = 1 hour
		double thresholdJoin = 0.5 * churn_factor / 24; //1/2 per day by default
		double thresholdLeave = 0; //0 per day by default.
		double thresholdGroupAdd = 1.0 * churn_factor / 24; //1 per day by default
		//double thresholdDeletion = 0;
		double thresholdRemove = 3 * del_factor / 24; //3 per day
		double thresholdGroupDeletion = 0.25 * del_factor / 24; //1/4 per day by default
		double thresholdObjectDeletion = 0.25 * del_factor / 24;
		double thresholdPost = 0.8 * post_freqm / 24; //1 per day by default.
		double thresholdaddO = 0.2 * post_freqm / 24; //1 per day by default.
		...
		if(coin <  thresholdPost) {
			...
			for(String a : actions) {
				if(a.endsWith("Add") || a.equals("addO")) {
