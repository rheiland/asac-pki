...
public class SkelW extends WorkloadScheme {

    public SkelW(WorkloadState initState) {
        super(initState);
    }
    ...
    private void command(Action acAction) {
        SkelWState rState = (SkelWState) state;
        Log.d("SkelW:command: action=" + acAction.name);
...
//                break;
    ...
    private void query(Action acAction) {
        SkelWState rState = (SkelWState) state;
        String u, r, p;
        Log.d("SkelW:query: action=" + acAction.name);
        switch(acAction.name) {
            case "SKEL": {
                Log.d("SkelW:query: action=" + acAction.name);
                return;
...
//        "UR", "PA", "auth"
    ...
    public Action addParameters(Action a, Workflow w) {
        SkelWState rState = (SkelWState) state;
        Log.d("SkelW:addParameters:  action=" + a.name);
        if(a.name.equals("addU")) {
            ...
            Log.w("SkelW: No recipe to add parameters to action " + a);
            return a;
        }
    }
}